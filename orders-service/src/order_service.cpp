...
models::Order OrderService::create_order(const std::string& user_id, 
    ...
    auto order_id = uuids::to_string(uuid_gen());
    ...
    db_->execute(tx, 
        ...
        std::chrono::system_clock::to_time_t(order.created_at));
...
std::vector<models::Order> OrderService::get_user_orders(const std::string& user_id) {
    ...
    for (const auto& row : result) {
        ...
        order.created_at = std::chrono::system_clock::from_time_t(
            row["created_at"].as<time_t>());
